...
namespace Fruit
{
    public class FruitPlayer : MonoBehaviour
    {
        ...
        {
            if (gamemanager.gameState == FruitGameManager.GAMESTATE.PLAYING)
            {
        ...
        public void EffectHit(GameObject objHit, Vector3 pos, Quaternion rotation, GameObject bullet)
        {
            ...
            EnemyFruitBullet health = objHit.GetComponent<EnemyFruitBullet>();

            // If there was a health script attached
            if (health != null && !health.isStartRemove)
            {
                // Call the damage function of that script, passing in our gunDamage variable
                health.OnHit();
            }
        ...
        public void TakeDamage(Vector3 posDamage, int damage)
        {
            hpPlayer -= damage;
            if (hpPlayer <= 0)
            {
